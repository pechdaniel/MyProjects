...
public class Controller {
    ...
    public Integer getMaxID(List<ProgramState> inPrgList) {
        Integer i = 1;
    ...
    public List<ProgramState> setAvailableForAll(List<ProgramState> inPrgList){
        Integer i=getMaxID(inPrgList)+1;
        inPrgList.stream().forEach(p->p.setAvailable(i));
        return inPrgList;
    }

    public void oneStepForAllPrgFX(List<ProgramState> prgList){
        try{
            prgList=setAvailableForAll(prgList);
            //prgList.stream().forEach(p->System.out.print(p.getID()+" "));
            //System.out.println();
            List<Callable<ProgramState>> callList = prgList.stream()
                    .map((ProgramState p) -> (Callable<ProgramState>)(() -> {p.getMylock().lock(); ProgramState p1=p.oneStep(); p.getMylock().unlock(); return p1;}))
                    .collect(Collectors.toList());
